...
model = tf.keras.models.load_model('object_detection/virusClassifierModel7.HDF5') 
...
def main(params):
	...
	db_query_to_insert_patient = 'insert into patient(user_id , patient_name , patient_age , patient_gen , patient_date) values(? , ? , ? , ? , ?);'
	cursor = conn.cursor()
	cursor.execute(db_query_to_insert_patient , (user_id , patient_name , patient_age , patient_gen, patient_date))
	conn.commit()
	print("---insert succes------")
	cursor.close()
	#conn.close()
	...
	base64ImgString = params['img64String'].encode()
	base64ImgString = base64.b64decode(base64ImgString)
	nparray = np.fromstring(base64ImgString , dtype = np.int8)
	...
	image , scores , boxes , classes = classify_infected(image_copy , scores , boxes , classes , threshold = 0.20)
	...
	#conn = getConnection()
	cursor = conn.cursor()
	db_query_to_insert_diagnosis = 'insert into inferences(inf_patient_id , inf_diagnosis) values(?,?);'
	cursor.execute(db_query_to_insert_diagnosis , (latest_patient_id , diagnosis))
	conn.commit()
	...
	#cv2.imwrite('C:/Users/Vaibhav/Desktop/Codes/BE Project/sourceCode/Object_detection2/models/research/object_detection/res.jpg' , image)
